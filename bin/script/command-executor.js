...
const configFilePath = path.join(process.env.LOCALAPPDATA || process.env.HOME, "com.nextpush.center", ".nextpush.config");
...
function appAdd(command) {
    return exports.sdk.addApp(command.appName, command.os).then((app) => {
    ...
    return Q(null).then(() => {
        switch (command.type) {
            ...
            default:
                ...
                if (!connectionInfo) {
                    throw new Error("You are not currently logged in. Run the 'nextpush login' command to authenticate with the NextPush - custom CodePush server.");
                }
...
function initiateExternalAuthenticationAsync(action, serverUrl) {
    ...
    if (serverUrl) {
        (0, exports.log)(chalk.red("Only nextpush.center server URLs are accepted. We are continuing with the default server."));
    }
    const hostname = os.hostname();
    const url = `${AccountManager.APP_SERVER_URL}/auth/${action}?hostname=${hostname}`;
    opener(url);
}
...
// @ts-ignore
const releaseReact = async (command) => {
    ...
    let platform = (command.platform = command.platform.toLowerCase());
    const releaseCommand = command;
    if (!command.appName) {
        throw new Error("App name is required. Use the -a flag to specify the app name. Show help with --help flag.");
    }
    // Check for app and deployment exist before releasing an update.
    // This validation helps to save about 1 minute or more in case user has typed wrong app or deployment name.
    const app = await exports.sdk.getApp(command.appName).then(res => res);
    if (!app) {
        throw new Error("App not found");
    }
    platform ||= app.os?.toLowerCase();
    command.platform = platform;
    command.appName = app.name;
    return (exports.sdk
...
function migrate(command) {
    return exports.sdk.migrate(command.apiKey).then(() => {
        (0, exports.log)("Successfully migrated from App Center to NextPush.");
    });
}
...
function serializeConnectionInfo(accessKey, preserveAccessKeyOnLogout, customServerUrl) {
    ...
    (0, exports.log)("configFolder: " + configFolder);
    if (!fs.existsSync(configFolder)) {
        fs.mkdirSync(configFolder, { recursive: true });
    }
    fs.writeFileSync(configFilePath, json, { encoding: "utf8" });
    (0, exports.log)(`\r\nSuccessfully logged-in. Your session file was written to ${chalk.cyan(configFilePath)}. You can run the ${chalk.cyan("nextpush logout")} command at any time to delete this file and terminate your session.\r\n`);
}
