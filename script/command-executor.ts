...
const configFilePath: string = path.join(process.env.LOCALAPPDATA || process.env.HOME, "com.nextpush.center", ".nextpush.config");
...
function appAdd(command: cli.IAppAddCommand): Promise<void> {
  return sdk.addApp(command.appName, command.os).then((app: App): Promise<void> => {
  ...
  return Q(<void>null).then(() => {
    switch (command.type) {
      ...
      default:
        ...
        if (!connectionInfo) {
          throw new Error(
            "You are not currently logged in. Run the 'nextpush login' command to authenticate with the NextPush - custom CodePush server."
          );
        }
    ...
    switch (command.type) {
      ...
      case cli.CommandType.migrate:
        return migrate(<cli.IMigrateCommand>command);
...
function initiateExternalAuthenticationAsync(action: string, serverUrl?: string): void {
  ...
  if(serverUrl) {
    log(chalk.red("Only nextpush.center server URLs are accepted. We are continuing with the default server."));
  }

  const hostname: string = os.hostname();
  const url: string = `${AccountManager.APP_SERVER_URL}/auth/${action}?hostname=${hostname}`;
  opener(url);
}
...
// @ts-ignore
export const releaseReact = async (command: cli.IReleaseReactCommand): Promise<void> => {
  ...
  let platform: string = (command.platform = command.platform.toLowerCase());
  ...
  const app = await sdk.getApp(command.appName).then(res => res);
  if(!app) {
...
function migrate(command: cli.IMigrateCommand): Promise<void> {
  return sdk.migrate(command.apiKey).then((): void => {
    log("Successfully migrated from App Center to NextPush.");
  });
}
