...
const USAGE_PREFIX = "Usage: nextpush";
...
export function showHelp(showRootDescription?: boolean): void {
  if (!wasHelpShown) {
    if (showRootDescription) {
      console.log(chalk.cyan(".__   __.  __________   ___ .___________.  " + chalk.green(".______    __    __       _______. __    __  ")));
      console.log(chalk.cyan("|  \\ |  | |   ____\\  \\ /  / |           |  " + chalk.green("|   _  \\  |  |  |  |     /       ||  |  |  | ")));
      console.log(chalk.cyan("|   \\|  | |  |__   \\  V  /  `---|  |----`  " + chalk.green("|  |_)  | |  |  |  |    |   (----`|  |__|  | ")));
      console.log(chalk.cyan("|  . `  | |   __|   >   <       |  |       " + chalk.green("|   ___/  |  |  |  |     \\   \\    |   __   | ")));
      console.log(chalk.cyan("|  |\\   | |  |____ /  .  \\      |  |       " + chalk.green("|  |      |  `--'  | .----)   |   |  |  |  | ")));
      console.log(chalk.cyan("|__| \\__| |_______/__/ \\__\\     |__|       " + chalk.green("| _|       \\______/  |_______/    |__|  |__| ")) + "    CLI v" + packageJson.version);
      console.log(chalk.cyan("===========================================") + chalk.green("============================================"));
      console.log("");
      console.log("NextPush is a service that enables you to deploy mobile app updates directly to your users' devices. \nvisit https://nextpush.center/ for more information\n");
    }
...
yargs
  ...
  .command("app", "View and manage your NextPush apps", (yargs: yargs.Argv) => {
    isValidCommandCategory = true;
    yargs
      ...
      .command("add", "Add a new app to your account", (yargs: yargs.Argv): void => {
        isValidCommand = true;
        yargs
          .usage(USAGE_PREFIX + " app add <appName> -o <os>")
          .demand(/*count*/ 1, /*max*/ 1) // Require exactly one non-option arguments
          .example("app add MyApp-iOS -o ios", 'Adds app "MyApp" for iOS')
          .example("app add MyApp-Android -o android", 'Adds app "MyApp" for Android')
          .option("os", {
            alias: "o",
            default: null,
            demand: true,
            description:
              "OS of the app to add. android or ios",
            type: "string",
          });
  ...
  .command("debug", "View the NextPush debug logs for a running app", (yargs: yargs.Argv) => {
    isValidCommandCategory = true;
    isValidCommand = true;
    yargs
      .usage(USAGE_PREFIX + " debug <platform>")
      .demand(/*count*/ 1, /*max*/ 1) // Require exactly one non-option arguments
      .example("debug android", "View the NextPush debug logs for an Android emulator or device")
      .example("debug ios", "View the NextPush debug logs for the iOS simulator");
  ...
  //   addCommonConfiguration(yargs);
  // })
  .command("login", "Authenticate with the NextPush server in order to begin managing your apps", (yargs: yargs.Argv) => {
    isValidCommandCategory = true;
    isValidCommand = true;
    yargs
      .usage(USAGE_PREFIX + " login [options]")
      .demand(/*count*/ 0, /*max*/ 1) //set 'max' to one to allow usage of serverUrl undocument parameter for testing
      .example("login", "Logs in to the NextPush server")
      ...
      .option("accessKey", {
        ...
        demand: false,
        description:
          "Access key to authenticate against the NextPush server with, instead of providing your username and password credentials",
        type: "string",
      })
      .check((argv: any, aliases: { [aliases: string]: string }): any => isValidCommand); // Report unrecognized, non-hyphenated command category.
  ...
  .command("register", "Register a new NextPush account", (yargs: yargs.Argv) => {
    isValidCommandCategory = true;
    isValidCommand = true;
    yargs
      .usage(USAGE_PREFIX + " register")
      .demand(/*count*/ 0, /*max*/ 1) //set 'max' to one to allow usage of serverUrl undocument parameter for testing
      .example("register", "Registers a new NextPush account")
      .check((argv: any, aliases: { [aliases: string]: string }): any => isValidCommand); // Report unrecognized, non-hyphenated command category.
  ...
  .command("release-react", "Release a React Native update to an app deployment", (yargs: yargs.Argv) => {
    yargs
      ...
      .demand(/*count*/ 0, /*max*/ 2) // Require exactly two non-option arguments
      .example(
  ...
  .command("migrate", "Import all your applications, CodePush versions, and metrics from App Center with a single command using your API key", (yargs: yargs.Argv) => {
    yargs
      .usage(USAGE_PREFIX + " migrate [options]")
      .demand(/*count*/ 0, /*max*/ 0) // Require exactly two non-option arguments
      .example("migrate --api-key <your-appcenter-api-key>", 'Import all your applications, CodePush versions, and metrics from App Center with a single command using your API key')
      .option("api-key", {
        alias: "k",
        default: null,
        demand: true,
        description:
          "App Center API key. You can get your App Center API key from [here](https://appcenter.ms/settings/apitokens/create).",
        type: "string",
      });
  ...
  if (!wasHelpShown && argv._ && argv._.length > 0) {
    ...
    switch (arg0) {
        ...
        switch (arg1) {
          case "add":
            if (arg2) {
              ...
              if (argv["os"]) {
                (<cli.IAppAddCommand>cmd).os = argv["os"] as any;
              } else {
                arg2.toLowerCase().includes("ios") ? (<cli.IAppAddCommand>cmd).os = "iOS" : (<cli.IAppAddCommand>cmd).os = "Android";
              }
            }
            break;
      ...
      case "release-react":
        ...
        releaseReactCommand.appName = argv["app"] as any || arg1;
      ...
      case "migrate":
        cmd = { type: cli.CommandType.migrate };
        const migrateCommand = <cli.IMigrateCommand>cmd;
        migrateCommand.apiKey = argv["api-key"] as any;
        break;
    }
